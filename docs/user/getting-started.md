...
2. optionally provide a backend-side schema config file for field metadata such as encrypted fields
3. point `e2ee-backend-adapter` at that manifest
...
The current scaffold validates the manifest, applies the optional backend schema config, connects to PostgreSQL, and exposes generated route placeholders plus adapter metadata endpoints.
